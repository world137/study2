	...
	for n in range(1,len(g)):
		...
		for i in range(len(line)):
			...
			for j in range(len(line[i])):
				...
				if(g[line[i][j][0]][line[i][j][1]]==g[line[i][j][1]][line[i][j][0]]):
					ans.append([line[i][j],2])
				else:ans.append([line[i][j],1])
			gSCC = Graph(len(newG))
...
for i in range(len(lst)):
    ...
    if len(scc) == 1:
    	...
    	ans.append(0)
    	line=addLine(newG(lst[i], scc))
    	for i in range(len(line)):
    		print(scc[line[i][0][0]][0],scc[line[i][0][1]][0],line[i][1])
